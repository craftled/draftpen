...
export function useLookoutForm(
  ...
  const handleUseExample = React.useCallback((example: any) => {
    ...
    setSelectedTimezone(
      example.timezone || (DEFAULT_FORM_VALUES.TIMEZONE as string)
    );
    setSelectedDayOfWeek(
      example.dayOfWeek || (DEFAULT_FORM_VALUES.DAY_OF_WEEK as string)
    );
  ...
  const populateFormForEdit = React.useCallback((lookout: any) => {
    ...
    if (lookout.cronSchedule) {
      ...
      const cronParts = parts[0]?.startsWith("CRON_TZ=")
        ? parts.slice(1)
        : parts;
      ...
      if (cronParts.length >= 2) {
        ...
        setSelectedTime(
          `${hours.padStart(2, "0")}:${minutes.padStart(2, "0")}` as string
        );
      }
  ...
  const createLookoutFromForm = React.useCallback(
    (formData: FormData, createLookout: any) => {
      ...
      const time = formData.get("time") as string;
      const timezone =
        (formData.get("timezone") as string) || DEFAULT_FORM_VALUES.TIMEZONE;
      const date = formData.get("date") as string;
  ...
  const updateLookoutFromForm = React.useCallback(
    (formData: FormData, updateLookout: any) => {
      ...
      updateLookout({
        ...
        frequency: frequency as "once" | "daily" | "weekly" | "monthly",
        time:
          frequency === "weekly" && dayOfWeek ? `${time}:${dayOfWeek}` : time,
        timezone,
        onSuccess: () => handleDialogOpenChange(false),
      });
    },
