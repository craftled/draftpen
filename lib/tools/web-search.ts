...
): item is SearchResultWeb => "url" in item && typeof item.url === "string";
...
const createSearchStrategy = (
  ...
  const strategies = {
    parallel: () =>
      new ParallelSearchStrategy(clients.parallel, clients.firecrawl),
    ...
    inputSchema: z.object({
      queries: z.array(
        z
          .string()
          .describe(
            "Array of 3-5 search queries to look up on the web. Default is 5. Minimum is 3."
          )
      ),
      maxResults: z
        .array(
