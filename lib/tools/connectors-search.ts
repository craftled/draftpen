...
export function createConnectorsSearchTool(
  ...
  return tool({
    description:
      "Search for documents in the user's connected services (Google Drive, Notion, OneDrive)",
    inputSchema: z.object({
      query: z.string().describe("The search query to find relevant documents"),
      provider: z
        .enum(
          availableProviders as [
            ConnectorProvider | "all",
            ...(ConnectorProvider | "all")[],
          ]
        )
        .optional()
        .describe(
          'Specific provider to search in, or "all" for all connected services'
        ),
    }),
    execute: async ({
      query,
        ...
        const enhancedResults = validResults.map((doc) => {
          ...
          return {
            ...
            providerConfig: detectedProvider
              ? CONNECTOR_CONFIGS[detectedProvider]
              : null,
            url: documentUrl,
          };
        });
