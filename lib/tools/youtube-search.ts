...
export const youtubeSearchTool = tool({
  ...
  execute: async ({
    ...
    try {
      ...
      const batchSize = 5;
      console.log(
        `📦 Creating batches from ${uniqueResults.size} unique videos with batch size ${batchSize}`
      );
      ...
      const batches = uniqueResultsArray.reduce(
        (
          acc: Array<Array<(typeof searchResult.results)[0]>>,
          result,
          index
        ) => {
          ...
          console.log(
            `➕ Added video ${index + 1} (${result.url}) to batch ${batchIndex + 1}`
          );
          return acc;
        },
      ...
      console.log(
        `📊 Batch creation complete: ${batches.length} batches created`
      );
      ...
      console.log(
        `🚀 Starting batch processing: ${batches.length} batches total`
      );

      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        const batch = batches[batchIndex];
        console.log(
          `🔄 [BATCH ${batchIndex + 1}/${batches.length}] Processing ${batch.length} videos`
        );
        console.log(
        ...
        try {
                ...
                try {
                  ...
                  const generateChaptersFromSubtitles = (
                    ...
                    for (
                      ...
                      const chosen =
                        idx >= 0 ? subs[idx] : subs[subs.length - 1];
                  ...
                  if (!timestamps) {
                    ...
                    if (subtitleSource && subtitleSource.length > 0) {
                      timestamps =
                        generateChaptersFromSubtitles(subtitleSource);
                    } else {
                      const subs = await getSubtitles({
                        videoID: videoId,
                        lang: "en",
                  ...
                  console.log(`✅ Successfully processed video ${videoId}:`, {
                    ...
                    timestampCount: Array.isArray(processedVideo.timestamps)
                      ? processedVideo.timestamps.length
                      : 0,
                    captionsLength: processedVideo.captions
                      ? processedVideo.captions.length
                      : 0,
                  });
          ...
          console.log(
            `  ✅ Successful: ${validBatchResults.length}/${batch.length}`
          );
          console.log(
            `  ❌ Failed: ${failedBatchResults.length}/${batch.length}`
          );

          if (failedBatchResults.length > 0) {
            console.log(
              "  🚨 Failed reasons:",
              failedBatchResults.map(
          ...
          console.log(
            `📈 [BATCH ${batchIndex + 1}] Total processed so far: ${processedResults.length} videos`
          );
          console.log(
          ...
          if (batchIndex < batches.length - 1) {
            console.log(
              `⏳ [BATCH ${batchIndex + 1}] Waiting 100ms before next batch...`
            );
            await new Promise((resolve) => setTimeout(resolve, 100));
          }
        } catch (batchError) {
          console.error(
            `💥 [BATCH ${batchIndex + 1}] CRITICAL BATCH FAILURE:`,
            batchError
          );
          if (batchError instanceof Error) {
            console.error(
              `💥 [BATCH ${batchIndex + 1}] Stack trace:`,
              batchError.stack
            );
          }
          console.log(
            `🔄 [BATCH ${batchIndex + 1}] Continuing to next batch despite failure...`
          );
        ...
        console.log(
          `✅ [BATCH ${batchIndex + 1}] Batch completed successfully`
        );
      }
      ...
      console.log(
        `🎯 Videos with content for UI: ${videosWithContent.length}/${processedResults.length}`
      );
