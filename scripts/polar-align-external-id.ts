...
async function main() {
  ...
  if (emailFlagIndex === -1 || !args[emailFlagIndex + 1]) {
    console.error(
      "Usage: bun run scripts/polar-align-external-id.ts --email <email>"
    );
    process.exit(1);
  }
  ...
  if (!customer) {
    console.error(
      "❌ No Polar customer found by email. Make sure this user has visited checkout/portal."
    );
    process.exit(1);
  }

  console.log(
    `✅ Found Polar customer id=${customer.id}, externalId=${customer.externalId ?? "NULL"}`
  );
  ...
  if (!customer.externalId) {
    ...
    console.log(
      `✅ Updated Polar customer externalId -> ${updated.externalId}`
    );
  } else if (customer.externalId !== user.id) {
    console.warn(
      "⚠️ Polar customer has a different externalId than our user.id"
    );
    console.warn(`   externalId=${customer.externalId} vs user.id=${user.id}`);
    console.warn(
      "   Per Polar: externalId cannot be changed once set. Options:"
    );
    console.warn(
      "   - Change our local user.id to match Polar (only safe at signup; risky post-signup)"
    );
  ...
  console.log(
    "   In Polar Dashboard → Webhooks, pick a subscription.updated/active event for this customer and click Resend."
  );
  console.log(
    "   This will upsert the subscription row with userId set, enabling Pro access (including trialing)."
  );
