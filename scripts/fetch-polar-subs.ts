...
async function main() {
  ...
  if (emailIdx === -1 && customerIdIdx === -1) {
    console.error(
      "Usage: bun run scripts/fetch-polar-subs.ts --customer-id <id> | --email <email>"
    );
    process.exit(1);
  }
  ...
  console.log(
    `Found ${subs.length} Polar subscription(s) for customer ${customerId}`
  );
  for (const s of subs) {
    console.log(
      `- id=${s.id} status=${s.status} currentPeriodEnd=${s.currentPeriodEnd} trial_start=${(s as any).trial_start} trial_end=${(s as any).trial_end}`
    );
  }
  process.exit(0);
}
