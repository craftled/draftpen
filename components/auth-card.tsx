...
const SignInButton = ({
  ...
  <button
    className="group relative flex h-12 w-full items-center justify-start gap-3 rounded-lg bg-muted/50 px-4 font-normal text-sm transition-all duration-200 hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
    disabled={loading}
    onClick={async () => {
      await signIn.social(
        {
...
export default function AuthCard({
  ...
  > = {
    github: { title: "GitHub", icon: <AuthIcons.Github className="h-4 w-4" /> },
    google: { title: "Google", icon: <AuthIcons.Google className="h-4 w-4" /> },
    twitter: { title: "X", icon: <AuthIcons.Twitter className="h-4 w-4" /> },
    microsoft: {
      title: "Microsoft",
      icon: <AuthIcons.Microsoft className="h-4 w-4" />,
    },
  };
